...
// global storage to store all the bytes received
std::vector<uint8_t> gAllBytesReceived;
...
CHIP_ERROR MockSubImageProcessor::Write(ByteSpan & block)
{
    ...
    if (strcmp(mName, "device-ready") == 0)
    {
        gAllBytesReceived.insert(gAllBytesReceived.end(), block.data(), block.data() + block.size());
    }
    return CHIP_NO_ERROR;
}
...
CHIP_ERROR MockSubImageProcessor::Finish()
{
    ...
    if (strcmp(mName, "device-ready") == 0)
    {
        printf("All bytes received: %s\n", gAllBytesReceived.data());
    }
    gAllBytesReceived.clear();
    return CHIP_NO_ERROR;
}
