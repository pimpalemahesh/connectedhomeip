...
{
    CHIP_ERROR err;
    MATTER_TRACE_INSTANT("MODE_NIMBLE_Init", "BLE");
...
{
    MATTER_LOG_METRIC(chip::Tracing::kMetricBluetoothConnections, mNumGAPCons);
    CancelBleAdvTimeoutTimer();
    MATTER_TRACE_INSTANT("Shutdown", "BLE");
...
void BLEManagerImpl::BleAdvTimeoutHandler(System::Layer *, void *)
{
    if (BLEMgrImpl().mFlags.Has(Flags::kFastAdvertisingEnabled))
    {
        ...
        MATTER_TRACE_INSTANT("Slow advertisement", "BLE");
    ...
    else
    {
        ...
        MATTER_TRACE_INSTANT("Extended advertisement", "BLE");
...
{
    esp_err_t err = ESP_OK;
    MATTER_TRACE_INSTANT("Deinit", "BLE");
...
CHIP_ERROR BLEManagerImpl::ConfigureBle(uint32_t aAdapterId, bool aIsCentral)
{
    ...
    if (mIsCentral)
    {
        MATTER_TRACE_INSTANT("CENTRAL", "BLE");
